...
class DefaultPreferences(
    ...
    override fun loadUserInfo(): UserInfo {
        val gender: Gender = Gender.fromString(
            sharedPref.getString(
                Preferences.KEY_GENDER,
                null
            )?: Gender.Male.name
        )
        val age: Int = sharedPref.getInt(
            Preferences.KEY_AGE,
            -1
        )
        val weight: Float = sharedPref.getFloat(
            Preferences.KEY_WEIGHT,
            -1f
        )
        val height: Int = sharedPref.getInt(
            Preferences.KEY_HEIGHT,
            -1
        )
        val activityLevel: ActivityLevel = ActivityLevel.fromString(
            sharedPref.getString(
                Preferences.KEY_ACTIVITY_LEVEL,
                null
            )?: ActivityLevel.Medium.name
        )
        val goalType: GoalType = GoalType.fromString(
            sharedPref.getString(
                Preferences.KEY_GOAL_TYPE,
                null
            )?: GoalType.KeepWeight.name
        )
        val carbRatio: Float = sharedPref.getFloat(
            Preferences.KEY_CARB_RATIO,
            -1f
        )
        val proteinRation: Float = sharedPref.getFloat(
            Preferences.KEY_PROTEIN_RATIO,
            -1f
        )
        val fatRatio: Float = sharedPref.getFloat(
            Preferences.KEY_FAT_RATIO,
            -1f
        )
        return UserInfo(
